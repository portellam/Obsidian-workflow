...
```dataviewjs
await dv.view("99 - Scripts", {pages: "dv.pages().file.where(f => f.folder != '_Sources' && f.folder != '04 - Archives' && f.folder != 'templates').tasks.where(t => !t.text.includes('🔁'))", view: "week", firstDayOfWeek: "1", options: "style8 filter", dailyNoteFolder: "10 - Daily"})
```
